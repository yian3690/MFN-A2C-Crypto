...
class MFN(nn.Module):
    def __init__(self, config, NN1Config, NN2Config, gamma1Config, gamma2Config, outConfig):
        ...
        [self.d_a, self.d_v] = config["input_dims"]
        [self.dh_a, self.dh_v] = config["h_dims"]
        total_h_dim = self.dh_a + self.dh_v
    ...
    def forward(self, x):
        ...
        for i in range(t):
            ...
            # concatenate
            prev_cs = torch.cat([prev_c_a, prev_c_v], dim=1)
            new_cs = torch.cat([new_c_a, new_c_v], dim=1)
        ...
        last_h_v = all_h_vs[-1]
        last_mem = all_mems[-1]
        last_hs = torch.cat([last_h_a, last_h_v, last_mem], dim=1)
