...
from src.evaluation_metrics import add_test_timestamps, validate_saved_result_period
...
class ExperimentPeriodsTestCase(unittest.TestCase):
    def test_expected_row_counts(self):
        self.assertEqual(EXPECTED_TOTAL_VALID_ROWS, 33_524)
        self.assertEqual(EXPECTED_TRAIN_ROWS, 32_444)
        self.assertEqual(TEST_ROWS, 1_080)
        self.assertEqual(EXPECTED_TRAIN_ROWS + TEST_ROWS, 33_524)
    ...
    def test_saved_result_requires_the_fixed_test_timeline(self):
        raw = pd.DataFrame(
            {"Open Time": pd.date_range(TEST_START, periods=TEST_ROWS, freq="2h")}
        )
        result = pd.DataFrame(
        ...
        self.assertEqual(len(stamped), TEST_ROWS - LOOKBACK)
