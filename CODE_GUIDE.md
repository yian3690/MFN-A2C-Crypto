...
- `src/experiment_config.py`：集中管理Train 31,364筆、Validation 1,080筆、300k上限、每50k依Validation Final PV選checkpoint的單階段流程、資料路徑、seed、paper DSR與四種方法共用設定。
...
四種方法均使用相同單階段流程。在31,364筆Train依時間順序訓練最多300,000步，每50,000步完整回測獨立Validation；Validation Final PV最高的checkpoint直接作為正式模型。Test不參與Scaler、訓練、特徵期限或checkpoint選擇。
