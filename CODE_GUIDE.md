...
2. `prepare_paper_features.py`：建立5個價格相對值、原四項指標與方案A的`RS_14D`，共5＋25維。正式Scaler只以31,904筆Train擬合，Validation與Test均不參與。
3. `check_alignment.py`：驗證train/validation/test的時間戳、原始價格及兩類特徵逐列對齊。
4. `src/experiment_periods.py`：集中定義2H間隔、Train／Validation／Test、20步lookback與預期筆數。正式切分為31,904／540／1,080筆。
...
目前將論文原32,444筆Train尾端540筆（45天）保留為Validation，實際Train為31,904筆；Test仍固定為2025-06-03 02:00至2025-09-01 00:00且最後才評估一次。
