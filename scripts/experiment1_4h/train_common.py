...
METHOD_LABELS = {
    ...
    "mfn": "MFN-A2C GITHUB-STYLE",
    "dman_attention": "DUAL-LSTM + DMAN + TEMPORAL SELF-ATTENTION A2C",
    "asset_mfn": "ASSET-WISE ATTENTION + MFN/MGM A2C",
    "self_attention": "ASSET-TEMPORAL SELF-ATTENTION A2C",
}
...
def _mfn_extractor_kwargs() -> dict:
    return {
...
def model_kwargs(method: str) -> dict:
    ...
    if method == "mfn":
        policy_kwargs.update(
            ...
            features_extractor_kwargs=_mfn_extractor_kwargs(),
        )
    elif method == "dman_attention":
        # 乾淨的MGM消融：保留原雙LSTM與DMAN，只以跨時間注意力取代MGM。
        policy_kwargs.update(
    ...
    elif method == "asset_mfn":
        extractor_kwargs = _mfn_extractor_kwargs()
        extractor_kwargs.update(
            indicators_per_asset=cfg.INDICATORS_PER_ASSET,
            asset_embedding_dim=16,
            asset_attention_heads=2,
            asset_attention_dropout=0.0,
        )
        policy_kwargs.update(
            features_extractor_class=AssetWiseAttentionMFN,
            features_extractor_kwargs=extractor_kwargs,
        )
    elif method == "self_attention":
        policy_kwargs.update(
            features_extractor_class=AssetTemporalSelfAttentionA2C,
            features_extractor_kwargs={
                "price_dim": cfg.PRICE_DIM,
                "indicator_dim": cfg.INDICATOR_DIM,
                "indicators_per_asset": cfg.INDICATORS_PER_ASSET,
    ...
    if method in {"mfn", "dman_attention", "asset_mfn", "self_attention"}:
        kwargs["policy_kwargs"] = policy_kwargs
        kwargs["device"] = cfg.MFN_DEVICE
    return kwargs
