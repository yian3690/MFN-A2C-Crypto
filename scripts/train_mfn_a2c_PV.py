"""以絕對Portfolio Value作reward訓練MFN-A2C並保存最後一步模型。"""
...
from src.pv_experiment import MFN_PV_MODEL_NAME, PV_RUN_TAG, make_pv_env
from src.training_diagnostics import TrainingDiagnosticsCallback


CHECKPOINTS = ROOT / "checkpoints_mfn_pv"
...
def diagnostics_callback() -> TrainingDiagnosticsCallback:
    ...
    path = LOGS / "training_diagnostics" / f"mfn_a2c_PV_{PV_RUN_TAG}_{stamp}.csv"
...
def main():
    ...
    checkpoint = CheckpointCallback(
        ...
        name_prefix=MFN_PV_MODEL_NAME.upper(),
    )
    output = MODELS / MFN_PV_MODEL_NAME

    print("=" * 64)
    print("MFN-A2C PV - FIXED FINAL TRAINING")
    print("=" * 64)
